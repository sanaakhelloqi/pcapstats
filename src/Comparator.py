...
class Comparator:
    ...
    def get_data_chi_squared(self, data_list_ori, data_list_aug):
        ...
        if n >= 35:
            ...
            out, bins = pd.qcut(data_list_ori, math.floor(n /5),
                                labels=False, retbins=True, duplicates='drop')
    ...
    def get_chi_squared_test(self):
        # get chi squared test for deltas
        ...
        # Visualise data for deltas frequencies
        self.viz_data[f"{Path(self.original.file).name},{Path(self.target.file).name}"]["Frequencies"]["plot_deltas"] \
            = {"x": list(bins[:-1]), "y1": [float(x) for x in list(histogram_original)],
               "y2": [float(x) for x in list(histogram_augmented)]}

        # get chi squared test for packets lengths
        ...
        # Visualise data for packets lengths
        self.viz_data[f"{Path(self.original.file).name},{Path(self.target.file).name}"]["Frequencies"]["plot_length"] =\
            {"x": list(bins_length[:-1]), "y1": [float(x) for x in list(hist_original_length)],
             "y2": [float(x) for x in list(hist_augmented_length)]}

        bins_packets_seconds, hist_packets_second, hist_augmented_packets_seconds = \
            self.get_data_chi_squared(self.original.get_packets_count_by_second(), self.target.get_packets_count_by_second())
        ...
        self.viz_data[f"{Path(self.original.file).name},{Path(self.target.file).name}"]["Frequencies"]["plot_packet_number"] = \
            {"x": list(bins_packets_seconds[:-1]), "y1": [float(x) for x in list(hist_packets_second)],
                                                               "y2": [float(x) for x in list(hist_augmented_packets_seconds)]}

    def get_data_KS_test(self, data_list_ori, data_list_aug):
        out, bins = pd.qcut(data_list_ori, math.floor(1.88 * len(data_list_ori) ** (2 / 5)), labels=False, retbins=True, duplicates='drop')
        ...
        return bins, cumulative_original, cumulative_augmented
    ...
    def get_kolmogorov_smirnov_test(self):
        ...
        self.comparisons["Kolmogorov_smirnov_test"]['Packet number by second'] = \
            stats.ks_2samp(self.target.get_packets_count_by_second(), self.original.get_packets_count_by_second()).pvalue

        bins, cumulative_original, cumulative_augmented = self.get_data_KS_test(self.original.get_deltas(), self.target.get_deltas())
        self.viz_data[f"{Path(self.original.file).name},{Path(self.target.file).name}"]["CDF"]["plot_deltas"] = \
            {"x": list(bins[:-1]), "y1": [float(x) for x in list(cumulative_original)],
        ...
        self.viz_data[f"{Path(self.original.file).name},{Path(self.target.file).name}"]["CDF"]["plot_length"] =\
            {"x": list(bins_lengths[:-1]), "y1": [float(x) for x in list(hist_lengths)],
             "y2": [float(x) for x in list(hist2_lengths)]}

        bins_packet_number, hist_packet_number, hist2_packet_number = self.get_data_KS_test(self.original.get_packets_count_by_second(),
                                                                          self.target.get_packets_count_by_second())
        self.viz_data[f"{Path(self.original.file).name},{Path(self.target.file).name}"]["CDF"]["plot_packet_number"] = \
            {"x": list(bins_packet_number[:-1]), "y1": [float(x) for x in list(hist_packet_number)],
